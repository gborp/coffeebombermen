...
public class GameCoreHandler implements ModelProvider, ModelController {
	...
	public GameCoreHandler(final GameManager gameManager, final MainFrame mainFrame, final ServerOptions globalServerOptions, final LevelModel levelModel,
	        final Random random, final Vector<PublicClientOptions> clientsPublicClientOptions, final int ourClientIndex) {
		...
		for (int y = 0; y < levelHeight; y++) {
			for (int x = 0; x < levelWidth; x++) {
				...
				if (y == 0 || y == levelHeight - 1 || x == 0 || x == levelWidth - 1) {
					...
					wall = random.nextInt(100) > 50 ? Walls.BRICK : Walls.CONCRETE;
		...
		while (changed) {
			...
			for (int y = 1; y < height - 1; y++) {
				for (int x = 1; x < width - 1; x++) {
					...
					if (!accessible[x][y] && levelComponents[y][x].getWall() != Walls.CONCRETE) {
						boolean hasAccessibleNeighbour = accessible[x - 1][y] || accessible[x + 1][y] || accessible[x][y - 1] || accessible[x][y + 1];
			...
			for (int y = 1; y < height - 1; y++) {
				for (int x = 1; x < width - 1; x++) {
					if (!accessible[x][y] && levelComponents[y][x].getWall() != Walls.CONCRETE) {
						...
						if (x > 1 && (vert || levelComponents[y][x - 1].getWall() != Walls.CONCRETE)) {
							levelComponents[y][x - 1].setWall(Walls.BRICK);
							isNowAccessible = isNowAccessible || accessible[x - 1][y];
						}
						if (x < width - 2 && (vert || levelComponents[y][x + 1].getWall() != Walls.CONCRETE)) {
							levelComponents[y][x + 1].setWall(Walls.BRICK);
							isNowAccessible = isNowAccessible || accessible[x + 1][y];
						}
						if (y > 1 && (horz || levelComponents[y - 1][x].getWall() != Walls.CONCRETE)) {
							levelComponents[y - 1][x].setWall(Walls.BRICK);
							isNowAccessible = isNowAccessible || accessible[x][y - 1];
						}
						if (y < height - 2 && (horz || levelComponents[y + 1][x].getWall() != Walls.CONCRETE)) {
							levelComponents[y + 1][x].setWall(Walls.BRICK);
							isNowAccessible = isNowAccessible || accessible[x][y + 1];
						}
						if (isNowAccessible) {
							accessible[x][y] = isNowAccessible;
						}
						neededToModify = true;
					}
				}
	...
	private void checkAndHandleBombDetonations() {
		...
		for (final BombModel bombModel : bombModels)
			if (bombModel.getPhase() != BombPhases.FLYING && !bombModel.isAboutToDetonate()
				...
				bombModel.setTriggererPlayer(levelComponents[bombModel.getComponentPosY()][bombModel.getComponentPosX()].fireModelVector.lastElement()
				        .getTriggererPlayer());
			}
