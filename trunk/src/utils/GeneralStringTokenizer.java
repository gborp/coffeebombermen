 ...
 * The tokens must be separated with the GENERAL_SEPARATOR_CHAR character. The
 * tokenizable string must last with a separator character to be able to gain
 * the last token.
 */
public class GeneralStringTokenizer {
	...
	/**
	 * General separator string separating things in strings (the general
	 * separator character in string format).
	 */
	 ...
	 * Note: remainingString() always returns those if hasMoreTokens() returns
	 * true, but nextXXXToken() methods only if the tokenized string terminates
	 * with GENERAL_SEPARATOR_CHAR.
	 * 
	 ...
	 * To corrigate means to replace the general separator character with the
	 * neutral replacer character. Since options may be stored in data text
	 * files, comment line characters must be also replaced.
	 * 
	 * @param string
	...
	public static String checkString(final String string) {
		return string.replace(GENERAL_SEPARATOR_CHAR, GENERAL_REPLACER_CHAR).replace(DataTextFileReader.COMMENT_LINE_CHAR, GENERAL_REPLACER_CHAR);
	}
