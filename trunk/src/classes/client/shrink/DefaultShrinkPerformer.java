	...
	protected void nextIterationImpl() {
		...
		if (isTimeToShrink()) {
			...
			if (isTimeToFirstShrink()
				...
				if (preSpeedupWarn > 0) {
					getLevel().getModel().getComponents()[lastNewWallY][lastNewWallX]
							.setWall(Walls.DEATH_WARN);
					preSpeedupWarn--;
					if (preSpeedupWarn == 0) {
				...
				if (newWallX >= 0 && newWallX < getWidth() && newWallY >= 0
						&& newWallY < getHeight()) {
					getLevel().getModel().getComponents()[newWallY][newWallX]
							.setItem(null);
					getLevel().getModel().getComponents()[newWallY][newWallX]
							.setWall(Walls.DEATH);
