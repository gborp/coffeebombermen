	...
	private void paintBombermen(final Graphics graphics) {
		...
		for (int i = 0; i < clientPlayerModels.size(); i++) { // Easy with the
			...
			for (int j = 0; j < playerModels.length; j++) {
				playerNoForGfx++;
				if (playerNoForGfx == playerGraphics.size()) {
					playerNoForGfx = 0;
				}
				...
				final Image bombermanImage = playerGraphics.get(playerNoForGfx).getImage(playerModel, scaleFactor);

				// Position is tricky: head of bomberman may take place on the
				// row over the position of bobmerman
				graphics.drawImage(bombermanImage, playerModel.getPosX() * levelComponentSize / LEVEL_COMPONENT_GRANULARITY - levelComponentSize / 2,
				        playerModel.getPosY() * levelComponentSize / LEVEL_COMPONENT_GRANULARITY + levelComponentSize / 2 - bombermanImage.getHeight(null),
				        null);
				...
				if (clientOptions.showPlayerNames) {
					...
					graphics.setColor(playerHudColor[playerNoForGfx]);
				...
				if (clientOptions.showBombermenLives) {
					...
					graphics.fillRect(playerModel.getPosX() * levelComponentSize / LEVEL_COMPONENT_GRANULARITY - levelComponentSize / 2 + 1, playerModel
					        .getPosY()
					        * levelComponentSize / LEVEL_COMPONENT_GRANULARITY + levelComponentSize / 2 + 3, levelComponentSize - 1, 4);
					graphics.setColor(Color.RED);
					graphics.fillRect(playerModel.getPosX() * levelComponentSize / LEVEL_COMPONENT_GRANULARITY - levelComponentSize / 2 + 1, playerModel
					        .getPosY()
					        * levelComponentSize / LEVEL_COMPONENT_GRANULARITY + levelComponentSize / 2 + 3, (levelComponentSize - 1)
