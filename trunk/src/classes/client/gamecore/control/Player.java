...
public class Player {
	...
	public void nextIteration() {
		if (model.getActivity() == Activities.DYING) {
			...
			useRegen++;
			handleSpyderBomb();
			if (useRegen == MAX_REGEN) {
				useRegen = 0;
				if (model.getVitality() < MAX_PLAYER_VITALITY*2) {
					model.setVitality(model.getVitality() + model.getRegenerateWeight() * 100);
	...
	private void processActionsAndHandleActivityTransitions() {
		...
		case STANDING:
			...
			if ((model.getControlKeyState(PlayerControlKeys.FUNCTION1)
					&& !model
							.getLastControlKeyState(PlayerControlKeys.FUNCTION1)) || model.isAutoDropBombEnabled())
	...
	private boolean determineNewDirection() {
		...
		if (model.getControlKeyState(PlayerControlKeys.DOWN)) {
			...
			if (!canPlayerStepToPosition(posX, posY
				...
				// correction
				// function...
				if (posX % LEVEL_COMPONENT_GRANULARITY < movementCorrectionSensitivity) // If
					// movement
					// correction
					// can
					// be
			...
			if (!canPlayerStepToPosition(posX
				...
				// correction
				// function...
				if (posY % LEVEL_COMPONENT_GRANULARITY < movementCorrectionSensitivity) // If
					// movement
					// correction
					// can
					// be
