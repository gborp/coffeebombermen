...
/**
 * Controlled timer.<br>
 * This class provides a confirmable timing service. The timed object must
 * implement the Timeable interface. The timing is based on the timing
 * frequency. If next iteration is about to come and confirmation has been
 * arrived that the timeable object is ready for the next iteration,
 * signalingNextIteration() will be called.<br>
 * So by default, we call signalingNextIteration() method in every T=1/f seconds
 * (if the timeable object is ready for it). If the timeable object is not
 * ready, we wait until it'll be ready, and the next signalingNextIteration()
 * call will be not sooner then T after the last call of
...
public class ControlledTimer extends ControlledThread {
	 ...
	 *            object to be timed
	 * @param frequency
	 *            the desirabled timing frequency
	 */
