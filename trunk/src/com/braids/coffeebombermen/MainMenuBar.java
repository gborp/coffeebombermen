...
public class MainMenuBar extends JMenuBar implements ActionListener {
	...
	private static class MID {
		...
		/**
		 * Position of character in the name to be the mnemonic for the menu
		 * item.
		 */
		 ...
		 * @param name
		 *            name of the menu item
		 * @param mnemonicCharPos
		 *            position of character in name to be the mnemonic for the
		 *            menu item
		 */
		 ...
		 *            menu item
		 * @param hasCheckbox
		 *            tells whether this menu item should be a checkbox menu
		 *            item
		 */
	...
	/**
	 * The descriptors of menus and menu items of the game (it's hierarchical,
	 * the first value is always the menu, the others are the menu items in the
	 * menu).
	 */
	...
	public enum GameStates {
		...
		IDLE,
		/**
		 * Player collecting phase, a server has been created, but we're not
		 * connected yet.
		 */
		PLAYER_COLLECTING_NOT_CONNECTED,
		/**
		 * Player collecting phase, a server has been created and we're
		 * connected to it.
		 */
	...
	/**
	 * The menu handler which has the appropriate methods to handle menu item
	 * actions.
	 */
	private final MainMenuHandler mainMenuHandler;
	/**
	 * Tells whether our server is running (whether we have a Server object
	 * created by us).
	 */
			...
			if (menuItems[menuItem.ordinal()] == eventSource) {
				switch (menuItem) {
					...
					case END_CURRENT_GAME:
						...
						break;
					case CLOSE:
						mainMenuHandler.closeGame();
						break;
					case EXIT:
						mainMenuHandler.exit();
						break;
					case CLIENT_OPTIONS:
						mainMenuHandler.showClientOptionsDialog();
						break;
					case SERVER_OPTIONS:
						mainMenuHandler.showServerOptionsDialog();
						break;
					case VIEW_GLOBAL_SERVER_OPTIONS:
						mainMenuHandler.showGlobalServerOptionsDialog();
						break;
	 ...
	 * 
	 * @param ourServerRunning
	 *            the new running state of our server
	 */
	...
	/**
	 * Packs the states of the checkbox menu items (the states of checkbox menu
	 * items) to a String.
	 * 
	 ...
	 * 
	 * @param menuStates
	 *            the packed menu states to be set
	 */
